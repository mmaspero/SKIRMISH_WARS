...
{
	return new move(data[FILA_OR_POS], data[COL_OR_POS], data[FILA_DES_POS], data[COL_DES_POS]);
}

attack * attackCreator(char * data, unsigned int size)
{
	return new attack(data[5],data[FILA_OR_POS], data[COL_OR_POS], data[FILA_DES_POS], data[COL_DES_POS]);
}
