...
bool cliente::ConectToServer(const char * ipServer, const char * portNumber)
{
	try 
	{
	...
	{
		return false;//la conexion no se pudo establecer
	}
	clienteSocket->non_blocking(true);
	return true;//la conexion se establecio con exito
...
//devuelve un false.
int cliente::receiveDataForServidor(char * buffer_t, int bufferSize)
{
...
int cliente::nonBlockinReceiveDataForServer(char * buffer_t, int bufferSize)
{
