...
bool contentBox::isItHere(unsigned int xPixel, unsigned int yPixel)
{
	return (contentStartX < xPixel && xPixel < contentStartX + contentWidth
		 && contentStartY < yPixel && yPixel < contentStartY + contentHeight);
}

button * contentBox::getButton(unsigned int xPixel, unsigned int yPixel)
{
...
displaySection_t contentBox::getType()
{
	return displaySectionType;
}
