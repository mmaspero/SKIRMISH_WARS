...
{
	float w = al_get_bitmap_width(unformattedAvatar), h = al_get_bitmap_height(unformattedAvatar);
	float sourceRadius = (float)min(w,h) / 2;
	roundAvatar = cropToCircle(unformattedAvatar, sourceRadius, radius - 3 * offset, w / 2, h / 2);
